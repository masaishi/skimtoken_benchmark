...
def benchmark_skimtoken_execution(
    texts: list[str], estimate_func: Any
) -> dict[str, Any]:
...
def calculate_rmse(true_counts: list[int], estimated_counts: list[int]) -> float:
    ...
    squared_errors = [
        (true - est) ** 2 for true, est in zip(true_counts, estimated_counts)
    ]
...
def calculate_error_rate(true_counts: list[int], estimated_counts: list[int]) -> float:
    ...
    percentage_errors = [
        abs(true - est) / true * 100
        for true, est in zip(true_counts, estimated_counts)
        if true > 0
    ]
...
def main() -> None:
    ...
    if args.dataset_path:
        ...
        dataset_path = Path(__file__).parent / "data" / "cc100_samples" / "test.jsonl"
    ...
    if args.num_items is not None:
        if args.num_items < len(valid_entries):
            ...
            console.print(
                f"[yellow]Randomly sampled {args.num_items} items from dataset[/yellow]"
            )
        else:
            console.print(
    ...
    console.print(
        f"[cyan]Benchmarking skimtoken.{args.method} initialization...[/cyan]"
    )
    ...
    skimtoken_exec = benchmark_skimtoken_execution(
        texts, skimtoken_init["estimate_func"]
    )
    ...
    error_rate = calculate_error_rate(
        tiktoken_exec["token_counts"], skimtoken_exec["token_counts"]
    )
    ...
    total_memory_skimtoken = (
        skimtoken_init["init_memory"] + skimtoken_exec["exec_memory"]
    )
    ...
    if category_metrics:
        ...
        sorted_categories = sorted(
            category_metrics.items(), key=lambda x: x[1]["error_rate"]
        )
